...
示例：你在系统学习“相对论”，中途分成 3 个方向：狭义相对论基础、广义相对论几何、实验验证与宇宙学。

```mermaid
flowchart TD
    A["同一个 AI 对话 URL<br/>主问题：我想系统学懂相对论，应该怎么开始？"]
    A --> B["分支1（Tab A）<br/>方向：狭义相对论（不包含分支2和3的内容）"]
    A --> C["分支2（Tab B）<br/>方向：广义相对论（不包含分支3的内容）"]
    A --> D["分支3（Tab C）<br/>方向：实验验证与宇宙学"]

    B --> B1["自动命名：时间膨胀与长度收缩"]
    C --> C1["自动命名：测地线与爱因斯坦场方程"]
    D --> D1["自动命名：引力透镜与引力波观测"]
```
