...
示例：你在系统学习“相对论”，先提出主问题，再分出 3 个子问题继续追问。

```mermaid
flowchart TD
    A["同一个 AI 对话 URL<br/>主问题：什么是相对论？"]
    A --> B["分支1（Tab A）<br/>子问题：光速不变原理是什么意思？"]
    A --> C["分支2（Tab B）<br/>子问题：迈克尔逊-莫雷实验是什么？"]
    A --> D["分支3（Tab C）<br/>子问题：质能方程怎么理解？"]

    B --> B1["自动命名：光速不变原理核心含义"]
    C --> C1["自动命名：迈克尔逊-莫雷实验结论"]
    D --> D1["自动命名：E=mc² 与能量转换"]
```
