...
`AI Chat Branch` 是一个 Chrome 插件，用来把标签页按“分叉”管理：  
在 AI 对话场景里，同一个会话 URL 复制成多个标签页后，每个标签页都可以沿不同方向继续追问，不再把所有问题混在一个对话里。  
...
对比官方分支能力（以 ChatGPT 为例）：

- 官方 Branch 通常会生成新的会话入口（URL 变化）
- 默认历史浏览更偏线性，父子层级关系不够直观
...
- 对话分叉
  ...
  - 手动粘贴同会话 URL 新开标签页时，也会自动推断其在节点树中的位置；但相比 Duplicate，父子关系推断稳定性会略低
